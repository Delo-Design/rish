...
if ! grep -q "MYSQLPASS" ~/.bashrc; then
  ...
  if ! check_step "$STEP"; then
    ...
    echo "12.3  со сроком поддержки до 12 июня 2029"
    echo
    echo "Какой релиз ставить?"
    while true; do
      Maria_Version_Custom=0
      vertical_menu "current" 2 0 10 default=1 "MariaDB 10.11" "MariaDB 11.4" "MariaDB 11.8" "MariaDB 12.3" "Ввести другую версию вручную"
      choice=$?
      case "$choice" in
      0)
        Maria_Version="10.11"
