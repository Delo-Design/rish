...
backup_crypto_generate_identity() {
    ...
    echo
    echo "Этот приватный ключ потребуется для восстановления зашифрованных бэкапов."
    echo "Без сохранённого ключа и его пароля расшифровать архивы невозможно."
    echo
    ...
    vertical_menu "current" 2 0 50 \
        "Сохранить на компьютер через буфер обмена" \
